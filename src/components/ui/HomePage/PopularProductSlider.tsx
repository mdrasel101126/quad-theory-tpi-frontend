...
const PopularProductSlider = ({slides}:any) => {
    const [productNumber,setProductNumber]=useState<number>(0);
    const [currentProduct,setCurrentProduct]=useState<number>(0);
    const popularProducts=slides?.filter((slide:any)=>slide.IsPopular==true);
    //console.log(slides);
    //console.log(products);
    const {products:newProducts}=useAppSelector(state=>state.product);
    //console.log(newProducts);
    const newPopularProducts=newProducts?.filter(p=>p.IsPopular);
    console.log(newPopularProducts);
    const products=[...popularProducts,...newPopularProducts]
